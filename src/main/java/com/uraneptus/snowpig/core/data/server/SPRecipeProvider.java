...
public class SPRecipeProvider extends RecipeProvider {
    ...
    public SPRecipeProvider(DataGenerator pGenerator) {
        super(pGenerator);
    }
    ...
    private static void modLoadedCookingRecipes(String modId, ItemLike ingredient, ItemLike result, float experience, String group, Consumer<FinishedRecipe> consumer) {
        ...
        ConditionalRecipe.builder()
                ...
                .addRecipe(consumer1 -> SimpleCookingRecipeBuilder.smelting(Ingredient.of(ingredient), result, experience, 200).group(group)
                        .unlockedBy(getHasName(ingredient), has(ingredient))
                        .save(consumer1, SnowPigMod.modPrefix(getItemName(result))))
                .build(consumer, smeltingPath(resultName));
        ConditionalRecipe.builder()
                .addCondition(new ModLoadedCondition(modId))
                .addRecipe(consumer1 -> SimpleCookingRecipeBuilder.campfireCooking(Ingredient.of(ingredient), result, experience, 600).group(group)
                        .unlockedBy(getHasName(ingredient), has(ingredient))
                        .save(consumer1, SnowPigMod.modPrefix(getItemName(result))))
                .build(consumer, campfire_cookingPath(resultName + "_from_campfire_cooking"));
        ConditionalRecipe.builder()
                .addCondition(new ModLoadedCondition(modId))
                .addRecipe(consumer1 -> SimpleCookingRecipeBuilder.smoking(Ingredient.of(ingredient), result, experience, 100).group(group)
                        .unlockedBy(getHasName(ingredient), has(ingredient))
                        .save(consumer1, SnowPigMod.modPrefix(getItemName(result))))
